...
function StatusBadge({ status }: { status: string }) {
    const map: Record<string, { bg: string; color: string; label: string }> = {
        ...
        paga: { bg: `${C.success}22`, color: C.success, label: "Paga" },
        atrasada: { bg: `${C.danger}22`, color: C.danger, label: "Atrasada" },
...
function SummaryCard({ icon, label, value, sub, highlight }: {
    ...
    return (
        <div className="rounded-2xl p-5 flex flex-col gap-2"
            style={{ backgroundColor: C.surface, border: `1px solid ${highlight ? C.danger + "60" : C.border}` }}>
            <div className="flex items-center gap-2" style={{ color: C.muted }}>
                <span className="text-lg">{icon}</span>
                <span className="text-xs font-medium">{label}</span>
            </div>
            <p className="text-2xl font-bold" style={{ color: highlight ? C.danger : C.text }}>{value}</p>
...
export default function DividasPage() {
    ...
    const fetchData = () => {
        ...
        setLoading(true);
        Promise.all([
            fetch(`${API_BASE}/contratos/${user.usuario_id}?tipo=pagar`).then(r => r.json()),
    ...
    const handleDelete = async (id: number) => {
        if (!confirm("Excluir esta dívida e todas as suas parcelas?")) return;
    ...
    const totalPagar = resumo?.total_pagar_mes ?? 0;
    const ativos = resumo?.contratos_pagar_ativos ?? 0;
    const proxVcto = resumo?.proximo_vencimento_pagar;

    return (
        <div className="min-h-screen font-sans antialiased" style={{ backgroundColor: C.bg, color: C.text }}>
            <AppNav activeItem="Dívidas" />
            ...
            <main className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-8">
                ...
                <div className="flex items-end justify-between mt-1 mb-6">
                    <div>
                        ...
                        <p className="text-sm mt-1" style={{ color: C.muted }}>Dívidas e contratos parcelados</p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                        <SummaryCard icon="💸" label="A pagar este mês" value={fmt(totalPagar)} highlight={totalPagar > 0} />
                        <SummaryCard icon="📅" label="Próximo vencimento" value={proxVcto ? fmtDate(proxVcto) : "—"} />
                        <SummaryCard icon="📋" label="Contratos ativos" value={String(ativos)} sub="dívidas em aberto" />
                    </div>
                )}
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20" style={{ color: C.muted }}>
                        ...
                        <p className="text-lg font-medium" style={{ color: C.text }}>Nenhuma dívida cadastrada</p>
                        <p className="text-sm mt-1">Clique em &quot;+ Nova Dívida&quot; para adicionar um contrato parcelado.</p>
                    </div>
                ) : (
                    <div className="flex flex-col gap-3">
                        {contratos.map(c => (
                            <div key={c.id}
                                ...
                                style={{ backgroundColor: C.surface, border: `1px solid ${c.parcelas_atrasadas > 0 ? C.danger + "50" : C.border}` }}>
                                ...
                                <div className="flex items-center gap-4 px-5 py-4 cursor-pointer hover:opacity-90 transition-opacity"
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs mt-0.5" style={{ color: C.muted }}>
                                            {c.parcelas_pagas}/{c.num_parcelas} parcelas pagas
                                            {c.proximo_vencimento && ` · Próximo: ${fmtDate(c.proximo_vencimento)}`}
                                        </p>
                                    </div>
                                    <div className="text-right flex-shrink-0">
                                        <p className="text-sm font-bold" style={{ color: C.danger }}>{fmt(c.valor_total)}</p>
                                        {c.proximo_valor && (
                                            <p className="text-xs" style={{ color: C.muted }}>Próxima: {fmt(c.proximo_valor)}</p>
                                        )}
                                    </div>
                                                ...
                                                <table className="w-full text-sm">
                                                    <thead>
                                                        <tr className="text-xs" style={{ color: C.muted, borderBottom: `1px solid ${C.border}` }}>
                                                            ...
                                                            <th className="px-5 py-2.5 text-left font-medium">Pago em</th>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/50 backdrop-blur-sm"
                    ...
                    <div className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[92dvh] overflow-y-auto"
                        ...
                        <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${C.border}` }}>
                            <div>
                                <h2 className="text-base font-bold">Nova Dívida</h2>
                                <p className="text-xs mt-0.5" style={{ color: C.muted }}>Preencha os dados do contrato</p>
                            </div>
                        ...
                        <form onSubmit={handleCreate} className="px-6 py-5 flex flex-col gap-4">
                            <div>
                                <label className="text-xs font-medium block mb-1.5" style={{ color: C.muted }}>Descrição / Credor *</label>
                            ...
                            {form.valor_total && form.num_parcelas && (
                                <div className="rounded-xl px-3 py-2.5 text-xs"
                                    style={{ backgroundColor: `${C.danger}12`, color: C.danger, border: `1px solid ${C.danger}30` }}>
            ...
            {baixaTarget && (
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/50 backdrop-blur-sm"
                    ...
                    <div className="w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-6 shadow-2xl"
                        style={{ backgroundColor: C.elevated, border: `1px solid ${C.border}`, animation: "modal-in 0.2s ease-out" }}
                        onClick={e => e.stopPropagation()}>
                        <h2 className="text-base font-bold mb-1">Confirmar Pagamento</h2>
                        <p className="text-sm mb-4" style={{ color: C.muted }}>
                            Marcar parcela <strong style={{ color: C.text }}>#{baixaTarget.numero_parcela}</strong> de{" "}
                            <strong style={{ color: C.danger }}>{fmt(baixaTarget.valor_parcela)}</strong>{" "}
                            (vencimento {fmtDate(baixaTarget.data_vencimento)}) como paga?
                        </p>
                        <div className="flex gap-3">
                            ...
                            <button onClick={handleBaixa} disabled={doingBaixa}
                                ...
                                {doingBaixa ? "Confirmando..." : "✓ Confirmar"}
                            </button>
                        </div>
                    </div>
                </div>
